...
using ResourceEdge.Logic.Auth_Providers;

namespace resourceEdge.webUi.Infrastructure
{
    public class DbInitializer : DropCreateDatabaseAlways<EdgeDbContext>
    {
        protected override void Seed(EdgeDbContext dbcontext)
        {
           // var userManager = new AuthManager.Core.AuthUserManager(new UserStore<AppUser, ApplicationRole, string, ApplicationUserLogin, ApplicationUserRole, ApplicationUserClaim>(new AuthManager.Core.AuthDbContext()));
           var  userManagerInstance = new AuthUserManagerProvider(new AuthManager.Core.AuthDbContext());
            var userManager = userManagerInstance.GetUserManager();
